...
void TSVLightPathTracing<kSampleSize>::traceCameraPath(RendererSystem& system)
{
  ...
  const auto sampled_wavelengthsX = makeTSVXLightStratifiedSampler<kXYZSampleSize>(world)(sampler);
  const auto sampled_wavelengthsY = makeTSVYLightStratifiedSampler<kXYZSampleSize>(world)(sampler);
  const auto sampled_wavelengthsZ = makeTSVZLightStratifiedSampler<kXYZSampleSize>(world)(sampler);
