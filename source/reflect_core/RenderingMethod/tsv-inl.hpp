...
TSVPathTracing<kSampleSize>::TSVPathTracing(RendererSystem& system,
...
void TSVPathTracing<kSampleSize>::render(RendererSystem& system)
{
...
/*!
  ...
  */
template <std::size_t kSampleSize>
void TSVPathTracing<kSampleSize>::evaluateExplicitConnection(
...
/*!
  ...
  */
template <std::size_t kSampleSize> inline
void TSVPathTracing<kSampleSize>::evaluateImplicitConnection(
    const double mis_weight,
    const Ray& ray,
...
/*!
  ...
  */
template <std::size_t kSampleSize> inline
Ray TSVPathTracing<kSampleSize>::generateRay(const std::size_t x, 
                                          const std::size_t y,
                                          Sampler& sampler,
                                          MemoryPool& memory_pool,
...
/*!
  ...
  */
template <std::size_t kSampleSize> inline
void TSVPathTracing<kSampleSize>::initialize(const QSettings& /* settings */)
{
}
...
/*!
  ...
  */
template <std::size_t kSampleSize>
Ray TSVPathTracing<kSampleSize>::sampleNextRay(const std::size_t path_length,
                                            const Ray& ray,
                                            const ShaderPointer& bxdf,
                                            const IntersectionInfo& intersection,
                                            Sampler& sampler,
...
フォトン1つ1つに波長を割り当てる
  */
template <std::size_t kSampleSize> inline
void TSVPathTracing<kSampleSize>::traceCameraPath(RendererSystem& system)
{
  ...
  {
    traceCameraPath(system, thread_id, y, sampled_wavelengthsXYZ);
  }};
                                               ...
                                               const std::tuple<SamplesXYZ, SamplesXYZ, SamplesXYZ>& sampled_wavelengthsXYZ)
{
  ...
  for (std::size_t i = 0; i < kXYZSampleSize; ++i)
  {
    wavelengths.setXYZwavelength(std::get<0>(sampled_wavelengthsXYZ).wavelengths()[i], i);
    wavelengths.setXYZwavelength(std::get<1>(sampled_wavelengthsXYZ).wavelengths()[i], i+kXYZSampleSize);
    wavelengths.setXYZwavelength(std::get<2>(sampled_wavelengthsXYZ).wavelengths()[i], i+kXYZSampleSize*2);
  }
...
//   {
//     traceCameraPath(system, thread_id, y, sampled_wavelengths);
//   }};
